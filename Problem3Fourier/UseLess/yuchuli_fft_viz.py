...
#     # Example with synthetic data (uncomment to test)
#     import numpy as np, pandas as pd
#     nu = np.linspace(1800, 3300, 1500)
#     y = 0.8*np.cos(2*np.pi*0.0032*nu + 1.1) + 0.4*np.cos(2*np.pi*0.0019*nu - 0.7)
#     # add baseline & absorption-like bump
#     y += 0.002*(nu-2550) + 0.5*np.exp(-0.5*((nu-2400)/50)**2)
#     df = pd.DataFrame({"nu": nu, "R": y})
#     visualize_fft_decomposition(df, include_range=(1800,3300),
#                                 exclude_ranges=[(2200,2300)],
#                                 k_components=5, show_max_components_in_fig1=5)
#     plt.show()

# ----------------------- Preprocessing comparison helper -----------------------
...
if __name__ == "__main__":
    ...
    out = preprocess_and_plot_compare(
        ...
        exclude_ranges=[(900, 1100)],   # 强吸收段（可多段）
